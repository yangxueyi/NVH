...
public class UseableAdapter extends RecyclerView.Adapter<UseableAdapter.MyViewHolder> {
    ...
    //清空列表
    public void deleteDevic(BluetoothDevice device){
        list.remove(device);
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        }else if(deviceType == 512){
            holder.iv_icon.setImageResource(R.mipmap.phone);
        }else if (deviceType == 0x0700){
        ...
        if (deviceName != null && deviceName.length() > 0) {
            ...
            holder.tv_name.setText(list.get(position).getAddress());
        }
    }
